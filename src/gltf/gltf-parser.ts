...
export class glTFParser {
  ...
  parseBuffer(accessor: any) {
    ...
    if (bufferView.byteStride !== undefined && bufferView.byteStride !== 0) {
      size =
        (bufferView.byteStride / componentSize[accessor.componentType]) *
          (accessor.count - 1) +
  ...
  decodeMeshoptBuffer(meshoptExtension: any) {
    ...
    MeshoptDecoder.decodeGltfBuffer(
      ...
      count,
      stride,
      new Uint8Array(meshoptBuffer, byteOffset, byteLength),
      meshoptExtension.mode,
  ...
  parseMaterial(material?: any) {
    ...
    if (material.occlusionTexture !== undefined) {
      result.occlusionTexture =
        this._asset.textures[material.occlusionTexture.index].clone();
      ...
      if (
        ...
        if (
          ...
          result.occlusionTexture.texCoord =
    ...
    if (material.normalTexture !== undefined) {
      result.normalTexture =
        this._asset.textures[material.normalTexture.index].clone();
      ...
      if (
        ...
        result.normalTexture.transform =
          material.normalTexture.extensions.KHR_texture_transform;
        if (
          material.normalTexture.extensions.KHR_texture_transform.texCoord !==
          undefined
        ) {
          result.normalTexture.texCoord =
            material.normalTexture.extensions.KHR_texture_transform.texCoord;
        }
      }
    }
    if (material.emissiveTexture !== undefined) {
      result.emissiveTexture =
        this._asset.textures[material.emissiveTexture.index].clone();
      ...
      if (
        ...
        result.emissiveTexture.transform =
          material.emissiveTexture.extensions.KHR_texture_transform;
        if (
          material.emissiveTexture.extensions.KHR_texture_transform.texCoord !==
          undefined
        ) {
          result.emissiveTexture.texCoord =
            material.emissiveTexture.extensions.KHR_texture_transform.texCoord;
        }
      }
    }
    ...
    if (pbr?.metallicRoughnessTexture !== undefined) {
      result.metallicRoughnessTexture =
        this._asset.textures[pbr.metallicRoughnessTexture.index].clone();
      result.metallicRoughnessTexture.texCoord =
        pbr.metallicRoughnessTexture.texCoord;
      if (
        pbr.metallicRoughnessTexture.extensions &&
        pbr.metallicRoughnessTexture.extensions.KHR_texture_transform
      ) {
        result.metallicRoughnessTexture.transform =
          pbr.metallicRoughnessTexture.extensions.KHR_texture_transform;
        if (
          pbr.metallicRoughnessTexture.extensions.KHR_texture_transform
            .texCoord !== undefined
          ...
          result.metallicRoughnessTexture.texCoord =
            pbr.metallicRoughnessTexture.extensions.KHR_texture_transform.texCoord;
        }
      }
    }
    ...
    if (pbr?.baseColorTexture !== undefined) {
      result.baseColorTexture =
        this._asset.textures[pbr.baseColorTexture.index].clone();
      ...
      if (
        ...
        result.baseColorTexture.transform =
          pbr.baseColorTexture.extensions.KHR_texture_transform;
        if (
          pbr.baseColorTexture.extensions.KHR_texture_transform.texCoord !==
          undefined
        ) {
          result.baseColorTexture.texCoord =
            pbr.baseColorTexture.extensions.KHR_texture_transform.texCoord;
        }
      }
    }
  ...
  parseMesh(mesh: any) {
    ...
    return <Mesh3D[]>mesh.primitives.map((primitive: any) => {
      return Object.assign<Mesh3D, Partial<Mesh3D>>(
        this.parsePrimitive(primitive),
        {
          name: mesh.name,
  ...
  parsePrimitive(primitive: any) {
    ...
    let geometry = Object.assign<MeshGeometry3D, Partial<MeshGeometry3D>>(
      new MeshGeometry3D(),
      {
  ...
  parseNode(index: number) {
    ...
    for (let skin of this._asset.descriptor.skins || []) {
      ...
      if (i >= 0) {
        ...
        const inverseBindMatrix = <Float32Array>(
          inverseBindMatrices?.buffer.slice(i * 16, i * 16 + 16)
        );
        joint = Object.assign<Joint, Partial<Joint>>(
          new Joint(inverseBindMatrix),
          {
            name: node.name,
          }
        );
    ...
    let container =
      joint ||
      Object.assign<Container3D, Partial<Container3D>>(new Container3D(), {
        name: node.name,
      });
    if (node.translation) {
  ...
  parseModel() {
    let nodes = <Container3D[]>this._descriptor.nodes.map(
      (n: any, i: number) => {
    ...
    let createHierarchy = (parent: Container3D, node: number) => {
      ...
      if (this._asset.descriptor.nodes[node].skin !== undefined) {
        skin = this.parseSkin(
          this._asset.descriptor.nodes[node].skin,
          nodes[node],
          nodes
        );
      }
